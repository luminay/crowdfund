...
import com.wf.crowd.config.PayProperties;
...
public class PayHandler {
    ...
    public String generateOrder(HttpSession session, OrderVO orderVO) throws AlipayApiException {
        ...
        // 计算总金额
        Double orderAmout = Double.valueOf(orderProjectVO.getSupportPrice() * orderProjectVO.getReturnCount() + orderProjectVO.getFreight());
        orderVO.setOrderAmount(orderAmout);

        session.setAttribute("orderVO", orderVO);

        //调用支付宝接口专门封装的方法
        return sendRequestToAliPay(orderNum, orderAmout, orderProjectVO.getProjectName(), orderProjectVO.getReturnContent());
    }
}
